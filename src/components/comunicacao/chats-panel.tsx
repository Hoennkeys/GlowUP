...
export function ChatsPanel({ initialChatId, draftMessage }: ChatsPanelProps = {}) {
  ...
  const filtradas = conversas.filter(
    (c) =>
      c.contatoNome.toLowerCase().includes(busca.toLowerCase()) ||
  ...
  const criarOportunidade = () => {
    ...
    toast.success(`Oportunidade criada: ${novo.cliente}`, {
      description: "Adicionada à etapa Sem Contato.",
    });
  };
  ...
  const salvarHistorico = () => {
    ...
    const resumo = ativa.mensagens
      .map((m) => `${m.autor === "agente" ? "Agente" : "Cliente"}: ${m.texto}`)
      .join(" | ");
  ...
  return (
    <Card className="grid grid-cols-12 h-full overflow-hidden p-0">
        ...
        <ScrollArea className="flex-1">
          {filtradas.length === 0 && (
            <p className="p-4 text-sm text-muted-foreground">
              Nenhuma conversa. Novos chats aparecerão aqui.
            </p>
          )}
          {filtradas.map((c) => (
            <button
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline gap-2">
                  ...
                  <span className="text-[10px] text-muted-foreground shrink-0">
                    {c.mensagens.length ? brTime(c.mensagens[c.mensagens.length - 1].em) : ""}
                  </span>
                </div>
              ...
              {c.naoLidas > 0 && (
                <Badge className="bg-emerald-600 text-white text-[10px] h-5">{c.naoLidas}</Badge>
              )}
            </button>
          ))}
        </ScrollArea>
          ...
          <>
            <div className="p-3 border-b bg-background flex items-center gap-3">
              ...
              <Badge variant="outline" className="text-xs">
                Atendido por {nomeVendedor(usuarios, ativa.agenteId)}
              </Badge>
            </div>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="space-y-2">
                <Button className="w-full" onClick={criarOportunidade}>
                  <UserPlus className="h-4 w-4" /> Criar Oportunidade no Funil
                </Button>
              ...
              <p className="text-xs text-muted-foreground">
                Última atividade:{" "}
                {brDate(ativa.mensagens[ativa.mensagens.length - 1]?.em ?? new Date())}
              </p>
            </div>
          </ScrollArea>
        )}
